...
def main():
    ...
    params = {
        ...
        "pageNo": "1",
        "type": "json",
        "prdctIdntNo": args.product_id,
    }

    print(f"=== prdctIdntNo={args.product_id} 로 날짜/업체명 필터 없이 직접 조회 ===")
    result = core.fetch(operation, service_key, params, log=print)
    payload = result["json"]
    ...
    print(f"totalCount={total_count}, 반환된 item 수={len(items)}")
