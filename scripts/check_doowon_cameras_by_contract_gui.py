#!/usr/bin/env python3
"""
check_doowon_cameras_by_contract.py를 위한 간단한 GUI.

업체명을 입력하면 1단계로 그 업체의 쇼핑계약번호를 찾고, 2단계로 그 계약번호에
딸린 물품 전체(추가선택품목 포함)를 가져온다. 전국 데이터를 스캔하지 않아서
어떤 회사를 검색하든 빠르다. 업체명/품명/규격/조회기간(1단계용)/서비스키를
화면에서 입력하고 "조회 시작" 버튼을 누르면 결과와 진행 로그를 창 안에서 볼 수 있다.
파이썬 표준 라이브러리(tkinter)만 사용하므로 별도 설치가 필요 없다.

실행:
    python3 scripts/check_doowon_cameras_by_contract_gui.py
"""
...
import check_doowon_cameras_by_contract as core


class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("업체 물품 조회 (계약번호로 전체 조회)")
    ...
    def _build_form(self):
        ...
        rows = [
            ("업체명 (계약업체명 기준)", self.corp_name_var, None),
            ("품명 (쉼표로 여러 개, 비우면 이 업체 물품 전체)", self.category_var, None),
            ("규격 (쉼표로 여러 개, 예: 200만화소,4배줌,블렛형)", self.spec_var, None),
            ("1단계(계약번호 탐색) 조회 시작일(YYYYMMDD)", self.begin_date_var, None),
            ("1단계(계약번호 탐색) 조회 종료일(YYYYMMDD)", self.end_date_var, None),
        ]
