#!/usr/bin/env python3
"""
check_doowon_cameras_corp_or_maker.py를 위한 간단한 GUI (계약업체명 OR 제조사).

계약업체명 또는 제조사 중 하나라도 입력한 이름을 포함하면 매칭되는 물품을 찾는다.
업체명(계약업체명/제조사 공용)/품명/규격/조회기간/서비스키를 화면에서 입력하고
"조회 시작" 버튼을 누르면 결과와 진행 로그를 창 안에서 볼 수 있다.
파이썬 표준 라이브러리(tkinter)만 사용하므로 별도 설치가 필요 없다.

실행:
    python3 scripts/check_doowon_cameras_corp_or_maker_gui.py
"""
...
import check_doowon_cameras_corp_or_maker as core


class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("두원전자통신 물품 조회 (계약업체명 OR 제조사)")
