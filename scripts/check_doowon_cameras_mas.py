...
"다수공급자계약 품목정보 조회"(getMASCntrctPrdctInfoList) 오퍼레이션으로 조회해서,
지정한 업체(기본값: 두원전자통신)가 등록한 영상감시장치가 몇 종류인지 확인하는 스크립트.

check_doowon_cameras.py는 getThptyUcntrctPrdctInfoList(제3자단가계약)를 쓰는데,
이 스크립트는 getMASCntrctPrdctInfoList(다수공급자계약, MAS)를 쓴다. 요청/응답
파라미터는 거의 동일하다(rgstDtBgnDt/rgstDtEndDt 등록일시 범위 1년 제한, prdctClsfcNoNm
품명 검색 등, 참고문서로 확인). 다만 cntrctCorpNm은 참고문서에 "Like 검색"이라는
설명이 없어서(다른 두 오퍼레이션과 달리) 완전일치일 수 있다 - 그래도 응답에 실제
포함된 회사명으로 클라이언트측 재필터링을 하므로 표기가 살짝 달라도 걸러진다.
...
사용법 (CLI):
    ...
    python3 scripts/check_doowon_cameras_mas.py
    python3 scripts/check_doowon_cameras_mas.py --corp-name "펜타게이트" --category "영상감시장치,보안용카메라"
    python3 scripts/check_doowon_cameras_mas.py --output 결과.xlsx

GUI로 실행하려면 scripts/check_doowon_cameras_mas_gui.py 를 실행하세요.
...
CANDIDATE_OPERATIONS = [
    "getMASCntrctPrdctInfoList",  # 다수공급자계약 품목 목록
]
