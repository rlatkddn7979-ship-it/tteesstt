...
"계약업체명(cntrctCorpNm)이 두원전자통신"이거나 "제조사(prdctMakrNm)가 두원전자통신"인
물품을 모두(OR) 찾는 스크립트.

check_doowon_cameras.py는 계약업체명만 보고, check_doowon_cameras_bymaker.py는 제조사만
보는데, 실제로는 두 경우가 다 있을 수 있다(두원전자통신이 직접 계약한 물품 / 두원전자통신이
만들고 다른 대리점이 계약한 물품). 이 스크립트는 그 둘을 합쳐서(OR) 보여준다.

동작 방식:
  1) 계약업체명(cntrctCorpNm)=업체명 으로 조회 (좁고 빠름, 서버에서 필터링)
  2) 품명(prdctClsfcNoNm)이 지정되어 있으면 그 품명으로, 안 되어 있으면 필터 없이(전국 데이터)
     조회한 뒤 응답에서 계약업체명 또는 제조사에 업체명이 들어있는 것만 클라이언트에서 골라낸다
  둘을 합쳐서 중복 제거한다.

주의: --category(품명)를 비워두면 2번 조회가 전국 모든 회사의 데이터를 다 받아와야 해서
매우 느리고 여러 페이지를 오래 조회하게 된다. 가능하면 품명을 지정하는 걸 권장하지만,
비워두고 싶다면 그대로 실행할 수 있다(시간이 오래 걸림을 감안할 것).
...
사용법 (CLI):
    ...
    python3 scripts/check_doowon_cameras_corp_or_maker.py
    python3 scripts/check_doowon_cameras_corp_or_maker.py --category ""   # 품명 없이 전체 조회(느림)

GUI로 실행하려면 scripts/check_doowon_cameras_corp_or_maker_gui.py 를 실행하세요.
...
def _fetch_paginated(
    operation, base_extra_params, date_chunks, num_of_rows, service_key,
    all_items, seen_keys, log, stage_label,
):
    """date_chunks 전체에 걸쳐 페이지를 넘겨가며 조회하고, all_items/seen_keys에 누적한다.
    (전체 조회 성공 여부를 나타내는 bool을 반환)"""
    any_success = False
    for chunk_idx, (chunk_begin, chunk_end) in enumerate(date_chunks, start=1):
        log(
            f"\n=== [{stage_label}] {operation} "
            f"(구간 {chunk_idx}/{len(date_chunks)}: {chunk_begin}~{chunk_end}) ==="
        )
        base_params = dict(base_extra_params)
        base_params.update({
...
def run_query(
    ...
    operation = CANDIDATE_OPERATIONS[0]

    # 1단계: 계약업체명으로 좁혀서 조회 (빠름)
    log(f"\n### 1단계: 계약업체명(cntrctCorpNm)='{corp_name}' 으로 조회 ###")
    ok1 = _fetch_paginated(
        operation, {"cntrctCorpNm": corp_name}, date_chunks, num_of_rows, service_key,
        all_items, seen_keys, log, "1단계:계약업체명",
    )
    ...
    if not any_success:
        log(
            "\n어떤 조회도 정상 응답(item 포함)을 주지 않았습니다. "
    ...
    log(f"\n1+2단계 합산(중복 제거) 조회된 전체 물품 수: {len(all_items)}")

    # 최종 매칭: 계약업체명 또는 제조사 중 하나라도 corp_name을 포함하면 매칭(OR).
