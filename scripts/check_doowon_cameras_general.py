...
"일반단가계약 품목정보 조회"(getUcntrctPrdctInfoList) 오퍼레이션으로 조회해서,
지정한 업체(기본값: 두원전자통신)가 등록한 영상감시장치가 몇 종류인지 확인하는 스크립트.

check_doowon_cameras.py는 getThptyUcntrctPrdctInfoList(제3자단가계약)를 쓰는데,
이 스크립트는 getUcntrctPrdctInfoList(일반단가계약)를 쓴다. 요청/응답 파라미터는
거의 동일하다(cntrctCorpNm Like검색, rgstDtBgnDt/rgstDtEndDt 등록일시 범위 1년 제한 등,
참고문서로 확인).
...
사용법 (CLI):
    ...
    python3 scripts/check_doowon_cameras_general.py
    python3 scripts/check_doowon_cameras_general.py --corp-name "펜타게이트" --category "영상감시장치,보안용카메라"
    python3 scripts/check_doowon_cameras_general.py --output 결과.xlsx

GUI로 실행하려면 scripts/check_doowon_cameras_general_gui.py 를 실행하세요.
...
CANDIDATE_OPERATIONS = [
    "getUcntrctPrdctInfoList",  # 일반단가계약 품목 목록
]
