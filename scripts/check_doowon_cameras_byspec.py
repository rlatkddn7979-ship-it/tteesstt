...
[규격(prdctSpecNm) 기준 검색 버전] check_doowon_cameras.py와 검색 조건(--category, --spec)은
동일하지만, 마지막 "종류 수" 집계 기준이 품명(prdctClsfcNoNm)이 아니라 규격/모델명
(prdctSpecNm)이다. 품명은 "보안용카메라"처럼 대분류라 서로 다른 모델이어도 다 같은 값으로
묶이지만, 규격은 물품마다 다른 자유 서술형 텍스트라 실제 "몇 종류(모델)"에 더 가깝다.
전체 품명 목록과 전체 규격 고유값 목록도 참고용으로 같이 보여준다.
평소에 쓰던 정식 버전(check_doowon_cameras.py)은 그대로 두고 이 파일만 새로 추가한 것.
...
사용법 (CLI):
    ...
    python3 scripts/check_doowon_cameras_byspec.py --category "" --spec ""
    python3 scripts/check_doowon_cameras_byspec.py --corp-name "펜타게이트" --category "보안용카메라,영상감시장치" --spec "200만화소,4배줌,블렛형"
    python3 scripts/check_doowon_cameras_byspec.py --output 결과.xlsx
...
def run_query(
    ...
    # "종류 수" 집계는 품명(대분류, NAME_FIELD)이 아니라 규격/모델명(SPEC_FIELD) 기준으로 한다.
    # 품명은 "보안용카메라"처럼 여러 모델이 다 같은 값으로 묶이는 대분류라 종류 수 집계에 안 맞는다.
    ...
    distinct_names = sorted({str(item.get(name_field, "")) for item in camera_items if item.get(name_field)})

    filter_desc = f"품명(OR)={category_terms or '(없음)'}, 규격(AND)={spec_terms or '(없음)'}"

    log(f"\n'{corp_name}' 전체 등록 물품 수: {len(all_items)}")
    log(f"조회 기간 내 등록된 전체 품명(참고용, {NAME_FIELD}) 종류 ({len(all_categories)}개):")
    ...
    if not camera_items:
        log(
            ...
            "위 전체 품명/규격 목록에서 정확한 표기를 확인하거나, 규격 조건을 줄여보세요. "
            "조회 기간(--begin-date/--end-date) 밖의 계약이라 안 보일 수도 있습니다."
        )
    log(f"일치하는 물품 종류(고유 {name_field} 개수, 규격 기준): {len(distinct_names)}")
