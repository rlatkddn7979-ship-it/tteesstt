#!/usr/bin/env python3
"""
check_doowon_cameras_shoppingmall_byspec.py를 위한 간단한 GUI.

getShoppingMallPrdctInfoList(나라장터 쇼핑몰 품목 정보 목록 조회) 오퍼레이션으로
조회하되, "종류 수"를 품명이 아니라 규격/모델명(prdctSpecNm) 기준으로 센다.
그래서 "보안용카메라"라는 같은 품명 안에서도 돔형/블렛형처럼 규격이 다른 물품을
서로 다른 종류로 자동 구분해서 보여준다. 계약업체명 또는 제조사 중 하나라도
입력한 이름을 포함하면 매칭되는 물품을 찾는다. 업체명/품명/규격/조회기간/서비스키를
화면에서 입력하고 "조회 시작" 버튼을 누르면 결과와 진행 로그를 창 안에서 볼 수 있다.
파이썬 표준 라이브러리(tkinter)만 사용하므로 별도 설치가 필요 없다.

실행:
    python3 scripts/check_doowon_cameras_shoppingmall_byspec_gui.py
"""
...
import check_doowon_cameras_shoppingmall_byspec as core


class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("두원전자통신 물품 조회 (나라장터 쇼핑몰, 규격 기준 종류 집계)")
    ...
    def _on_done(self, result, saved_path):
        ...
        if result["ok"]:
            ...
            messagebox.showinfo(
                ...
                f"종류 수(규격 기준): {len(result['distinct_names'])}개\n"
