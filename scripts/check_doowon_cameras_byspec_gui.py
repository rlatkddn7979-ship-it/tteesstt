#!/usr/bin/env python3
"""
check_doowon_cameras_byspec.py를 위한 간단한 GUI (규격 기준 종류 집계).

회사명/품명/규격/조회기간/서비스키를 화면에서 입력하고
"조회 시작" 버튼을 누르면 결과와 진행 로그를 창 안에서 볼 수 있다.
마지막 "종류 수"가 품명이 아니라 규격/모델명(prdctSpecNm) 기준으로 집계된다.
파이썬 표준 라이브러리(tkinter)만 사용하므로 별도 설치가 필요 없다.

실행:
    python3 scripts/check_doowon_cameras_byspec_gui.py
"""
...
import check_doowon_cameras_byspec as core


class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("두원전자통신 물품 조회 (규격 기준 종류 집계)")
