...
[규격(prdctSpecNm) 진단용 버전] check_doowon_cameras.py와 동일하지만, --spec을 비워두고
돌렸을 때 품명(prdctClsfcNoNm)뿐 아니라 규격/모델명(prdctSpecNm)의 전체 고유값 목록도
같이 출력한다 - 실제로 어떤 규격 텍스트들이 등록돼 있는지 눈으로 확인해보기 위한 테스트용.
...
사용법 (CLI):
    ...
    python3 scripts/check_doowon_cameras_spec.py --category "" --spec ""
    python3 scripts/check_doowon_cameras_spec.py --corp-name "펜타게이트" --category "보안용카메라,영상감시장치" --spec "200만화소,4배줌,블렛형"
    python3 scripts/check_doowon_cameras_spec.py --output 결과.xlsx
