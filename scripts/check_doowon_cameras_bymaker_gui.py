#!/usr/bin/env python3
"""
check_doowon_cameras_bymaker.py를 위한 간단한 GUI (제조사 기준 검색).

계약업체명이 아니라 "제조사"로 필터링한다. 제조사/품명/규격/조회기간/서비스키를
화면에서 입력하고 "조회 시작" 버튼을 누르면 결과와 진행 로그를 창 안에서 볼 수 있다.
파이썬 표준 라이브러리(tkinter)만 사용하므로 별도 설치가 필요 없다.

실행:
    python3 scripts/check_doowon_cameras_bymaker_gui.py
"""
...
import check_doowon_cameras_bymaker as core


class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("두원전자통신 물품 조회 (제조사 기준)")
    ...
    def _build_form(self):
        ...
        self.maker_name_var = tk.StringVar(value="두원전자통신")
    ...
    def _start_query(self):
        ...
        maker_name = self.maker_name_var.get().strip()
        ...
        self.worker = threading.Thread(
            ...
            args=(maker_name, category, spec, begin_date, end_date, service_key),
            daemon=True,
        )
        self.worker.start()

    def _run_in_background(self, maker_name, category, spec, begin_date, end_date, service_key):
        try:
            result = core.run_query(
                maker_name, category, spec, begin_date, end_date, service_key, log=self._log
            )
            saved_path = None
            if result["ok"]:
                output_path = core.default_output_filename(maker_name, category, spec)
                saved_path = core.write_output(
                    result["camera_items"], maker_name, category, spec, begin_date, end_date,
