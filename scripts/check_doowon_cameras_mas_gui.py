#!/usr/bin/env python3
"""
check_doowon_cameras_mas.py를 위한 간단한 GUI.

getMASCntrctPrdctInfoList(다수공급자계약 품목정보 조회) 오퍼레이션으로 조회한다.
회사명/품명/규격/조회기간/서비스키를 화면에서 입력하고
"조회 시작" 버튼을 누르면 결과와 진행 로그를 창 안에서 볼 수 있다.
파이썬 표준 라이브러리(tkinter)만 사용하므로 별도 설치가 필요 없다.

실행:
    python3 scripts/check_doowon_cameras_mas_gui.py
"""
...
import check_doowon_cameras_mas as core


class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("두원전자통신 물품 조회 (다수공급자계약)")
