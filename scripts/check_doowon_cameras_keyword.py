...
[키워드 통합검색 버전] 품명/규격을 따로 입력하지 않고, 키워드 하나로 품명(prdctClsfcNoNm)과
규격(prdctSpecNm) 양쪽을 한꺼번에 검색한다. 회사명/품명/규격을 각각 따로 입력하려면
check_doowon_cameras.py를 사용하세요.
...
사용법 (CLI):
    ...
    python3 scripts/check_doowon_cameras_keyword.py
    python3 scripts/check_doowon_cameras_keyword.py --corp-name "다른업체명" --keyword "CCTV"
    python3 scripts/check_doowon_cameras_keyword.py --output 결과.xlsx

GUI로 실행하려면 scripts/check_doowon_cameras_keyword_gui.py 를 실행하세요.
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="페이지당 조회 건수. numOfRows가 크면(예: 999) 서버 쿼리가 무거워져 타임아웃이 "
             "잘 나는 게 확인돼서 기본값을 낮게 잡음. 여러 페이지로 나눠 전부 가져온다",
    )
    parser.add_argument(
        "--begin-date",
