#!/usr/bin/env python3
"""
조달청 나라장터 종합쇼핑몰 물품정보 서비스(ShoppingMallPrdctInfoService)의
"나라장터 쇼핑몰 품목 정보 목록 조회"(getShoppingMallPrdctInfoList) 오퍼레이션으로 조회한다.

지금까지 쓰던 check_doowon_cameras*.py 계열은 전부 getThptyUcntrctPrdctInfoList
(제3자계약 품목정보 조회)를 썼는데, 이건 "제3자단가계약의 선택부품(기본 등록 품목)"만
다루는 것으로 보이고, 실제 쇼핑몰에서 구매 가능한 "추가선택품목"까지 포함한 전체 목록은
getShoppingMallPrdctInfoList가 따로 담당하는 것으로 보인다(참고문서 확인).
이 오퍼레이션의 응답 규격 형식(prdctSpecNm="품명, 제조사, 모델명, 규격" 스타일)이
나라장터 쇼핑몰 사이트 화면과 일치해서, 사이트에서 보이지만 기존 방식으로는 검색이 안 되던
품목(예: 추가선택품목으로 등록된 네트워크스위치 등)을 이걸로 찾을 수 있는지 확인하는 스크립트.

이 오퍼레이션도 계약업체명을 직접 검색하는 파라미터가 없어서, 품명(prdctClsfcNoNm)으로
서버 조회 범위를 좁힌 뒤 계약업체명(cntrctCorpNm) 또는 제조사(prdctMakrNm) 중
하나라도 업체명을 포함하면(OR) 클라이언트에서 걸러낸다.

등록일자 조회 범위는 한 번에 최대 12개월까지만 가능해서(참고문서), 그보다 긴 기간은
자동으로 여러 구간으로 나눠 조회한 뒤 합친다.
...
사용법 (CLI):
    ...
    python3 scripts/check_doowon_cameras_shoppingmall.py
    python3 scripts/check_doowon_cameras_shoppingmall.py --category "" --begin-date 20200101

GUI로 실행하려면 scripts/check_doowon_cameras_shoppingmall_gui.py 를 실행하세요.
...
SPEC_FIELD = "prdctSpecNm"  # 물품규격명 (자유 서술형 설명)
